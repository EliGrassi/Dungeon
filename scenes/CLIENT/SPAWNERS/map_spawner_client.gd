...
func _ready() -> void:
	...
	SceneChangeHandler.server_confirmed_signal.connect(scene_changed)
...
func enter_first_area(player_id: int) -> void:
	SceneChangeHandler.start_scene_change.emit(player_id, 0)
	
#Collect the UID we want based on the scene_id confirmed to be ours by the server
#Call the function to load this map UID
func scene_changed(peer_id: int, scene_id: int, _leaving: int) -> void:
	if !peer_id == ClientGlobals.id: return
