...
func change_scene(body: PhysicsBody2D) -> void:
	...
	if body.get_parent() != null and body.get_parent().name == str(ClientGlobals.id):
		#use 3 to mean nothing for the leaving value
		SceneChange.create(id, target_scene, 3).send(ClientNetworkHandler.connected_server)
